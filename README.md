...
2. Install [Termux](https://f-droid.org/en/packages/com.termux/) app from F-Droid (NOT from the Play Store).
...
```
mkdir -p $HOME/ObsidianSync
cp "/storage/emulated/0/repos/obsidian-android-sync/setup" ~/ObsidianSync/ && chmod +x "$HOME/ObsidianSync/setup" && source "$HOME/ObsidianSync/setup"
```
5. The above command copied an SSH public key to your clipboard (or was displayed to the screen), paste this into your Git host's SSH key authentication setting (eg [Github](https://github.com/settings/keys)). If you want to copy the SSH key again, run the setup script again by simply running `setup`. The long verison of the setup command (above) is noteeded anymore.
